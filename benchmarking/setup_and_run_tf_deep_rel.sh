...
for dir in $(find "$ROOT_DIR" -type d); do
    if echo "$dir" | grep -Eq "neq" || echo "$dir" | grep -Eq "bug"; then
        ...
        do
            find "$file" -name "*.py" -exec sh -c 'echo "Processing file: $1"; python "$1"' sh {} \; |& tee -a "/media/nimashiri/DATA/vsprojects/benchmarkingDLFuzzers/logs/DeepRel/tensorflow/$tf_version.txt";
        done
    fi
done
